...
The research follows the Design Science Research (DSR) paradigm \cite{hevner2004}, which prescribes the iterative construction and evaluation of artifacts that address identified problems. In the knowledge-contribution framing of Gregor and Hevner \cite{gregor2013}, this work occupies the \emph{improvement} quadrant: a new solution architecture --- rights-grounded, incident-evidenced, scrutable multi-framework assessment --- for a known problem, early-stage research ethics compliance. The contribution is therefore twofold: the artifact itself, and the design knowledge the evaluation produces about \emph{when and where} knowledge-graph grounding and symbolic rules improve on LLM-only assessment. The work maps onto the six activities of the DSR methodology of Peffers et al.\ \cite{peffers2007}: problem identification (Chapter~\ref{ch:introduction}), solution objectives (the research questions), design and development (this chapter), demonstration (the deployed application and real-world cases), evaluation (Chapter~\ref{ch:evaluation}), and communication (this dissertation and the public artifacts). The design cycle proceeded through five iterations: (1) framework extraction and requirement harmonisation, (2) ontology design and expert review, (3) knowledge graph population and validation, (4) RAG pipeline construction, and (5) evaluation and refinement.
...
The ontology comprises 125 classes, 22 object properties, and 16 data properties. Table~\ref{tab:ontology_summary} summarises the key structural elements.
...
Total asserted triples & 3,931$^{*}$ \\
Total triples including inferred (GraphDB, RDFS+, production ruleset) & 4,863$^{*}$ \\
\bottomrule
\end{tabular}

\vspace{2mm}
\footnotesize $^{*}$All evaluation results in Chapter~\ref{ch:evaluation} (RAGAS metrics, retrieval baselines, error taxonomy, rights coverage) were computed against this 70-incident, 3,931-triple state. Three additional incidents (73 total, 3,961 asserted triples) were added on 16 July 2026 as a targeted stress test of the RQ1 landscape-analysis gap finding (\S\ref{sec:landscape}) and are scoped to that analysis only; they postdate and are excluded from every other reported evaluation figure to keep those results comparable to their originally-reported values.
\end{table}
...
Quantitatively, the ontology asserts 15 \texttt{owl:equivalentClass} alignments to DPV rights terms and 3 \texttt{rdfs:subClassOf} alignments to ODRL deontic classes, referencing 18 distinct external vocabulary terms in total --- a deliberate reuse-over-reinvention posture for the rights and policy layers, while the requirement, incident, and evidence vocabularies are AIEF-authored because no existing vocabulary covers the four-framework harmonisation.
...
\subsection{Competency-Question Verification and Reasoner Consistency}
\label{sec:cq-validation}

Following the competency-question methodology of Gr\"{u}ninger and Fox \cite{gruninger1995}, twenty formal competency questions were authored spanning the ontology's intended capabilities --- requirement lookup by topic, right, framework, tier, evidence demand, and legal basis; incident lookup by right, capability, and requirement support; and cross-framework structural queries. Each question was translated to a SPARQL query with an explicit pass criterion: seventeen require a non-empty answer set, and three assert exact values that cross-validate findings reported independently elsewhere in this dissertation (the incident-impacted rights lacking requirement coverage must be exactly Articles 13, 15, and 48; the rights covered by all four frameworks must number exactly 8; the DPV-aligned rights must number exactly 15). All twenty questions pass against the live knowledge graph (Table~\ref{tab:cq}). Separately, the full ontology was checked with the HermiT OWL reasoner, which completed classification and reported zero inconsistent classes.

\begin{table}[htbp]
\centering
\caption{Competency-question verification results (20/20 pass, 16 July 2026).}
\label{tab:cq}
\small
\input{project/cq_table.tex}
\end{table}
...
A recurring critique of the main pipeline is that a single High/Medium/Low label collapses heterogeneous risk, and that the ranking rationale for that label sits inside the LLM. The SHACL layer in \texttt{shacl/} demonstrates the alternative: 21 risk rules across 20 of the ontology's 22 Charter dimensions, each expressed as a SHACL shape declaring as data its risk dimension (an EU Charter right IRI), priority rank (\texttt{aiefsh:priority}), severity tier (\texttt{sh:severity}), written rationale (\texttt{rdfs:comment}), and an inspectable SPARQL constraint. Validation produces a \emph{per-dimension risk profile} ordered by priority rather than one aggregate label. No LLM is involved in this layer. The rule set was widened from an initial 6 rules/4 dimensions specifically to withstand the objection that a narrow demonstrator cannot be fairly compared against the full RAG pipeline (\S\ref{sec:ablation}); the two dimensions left unruled (Art.~13, Art.~20) are a stated scope limit, justified there.

This design is not merely an engineering convenience; it operationalises a specific position in the AI governance literature on contestability. Alfrink et al.\ \cite{alfrink2023contestable} synthesise a design framework for AI systems that are \emph{contestable by design} --- responsive to human intervention throughout the system lifecycle, not merely appealable after the fact --- and identify inspectable, revisable decision logic as a core socio-technical feature that contestability requires. Priorities expressed as editable RDF triples, re-validated on demand rather than re-trained, are a direct instantiation of that feature. Hildebrandt's earlier argument for \emph{agonistic machine learning} \cite{hildebrandt2019agonistic} makes a related, more foundational claim: that bringing data-driven systems under the rule of law requires a research design in which those affected by an algorithm can meaningfully debate and contest the modelling choices that represent them, rather than a purely agnostic system that treats its own classifications as given. The SHACL layer's harm-evidenced priorities (below) and its demonstrated re-ranking under a changed institutional priority (Chapter~\ref{ch:evaluation}, time-to-update trial) are one way of making that agonistic debate operational: an evaluator who disagrees with a priority is not merely heard, they change the artefact and re-run it.
...
An evaluator who weighs clinical risk more heavily edits two triples on \texttt{ClinicalValidationShape} (\texttt{aiefsh:priority 2}$\rightarrow$\texttt{1}, \texttt{sh:severity} Warning$\rightarrow$Violation) and re-runs \texttt{run\_shacl.py}: P08's clinical-validation finding moves to the top tier of its profile. The same override can be packaged as an institution profile file (\texttt{profiles/health-faculty.ttl}) and applied with \texttt{--profile}, so institutional risk cultures become version-controlled data rather than prompt edits. The free-text bridge (\texttt{text\_to\_description.py}) materialises structured descriptions from proposal text using the same keyword heuristics as retrieval, so SHACL can run end-to-end on free text; negation handling remains naive (regex, not NLU) --- a known limitation of the same class as keyword retrieval. Evaluated over all 20 synthetic proposals, the six-rule set achieves mean precision 0.417 and recall 0.193 against expected rights --- high relative to its four-dimension bound, low in absolute terms, which is the intended RQ2 trade-off.
