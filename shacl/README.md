...
**Attempted ablation, not completed (19 July 2026).** `text_to_description_llm.py` implements an alternative bridge using Groq-based structured extraction instead of regex, to test whether replacing the naive bridge measurably improves SHACL recall — at the cost of reintroducing LLM dependency at the extraction stage, undermining the "zero LLM involvement" property of the SHACL layer specifically in exchange for coverage. `bridge_ablation.py` scores both bridges identically via `evaluate_shacl.py`'s methodology. The code is complete and the one successful comparison obtained (P14, both bridges scoring 0.00/0.00) is consistent but is a single data point, not an ablation result. The remaining 19 comparisons failed against the same Groq free-tier daily token cap (100,000 TPD) documented elsewhere in this project for the Qwen3 exploratory run and the repeated-run variance study — the quota was already near-exhausted by those attempts before this ablation ran. Completing this comparison on a fresh daily quota (or a paid tier) is recorded as a specific, ready-to-run future-work item: the code requires no further changes, only quota.
